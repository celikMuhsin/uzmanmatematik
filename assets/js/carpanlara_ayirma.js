...
const SoruMotoru = {
    ...
    uretSeviye3: function () {
        ...
        // STRATEJİ: DENGELİ DAĞILIM (5 ŞIK: 1 Doğru + 4 Yanlış)
        // 1. Ters İşaret (Mevcut m ve n sayıları karışmasın diye işaretleri değiştir)
        hamYanlislar.push(`(x ${formatla(-m)})(x ${formatla(-n)})`);

        // 2. Akıllı Çeldirici (Smart Distractor) - b^2 + 4c (Sabit terim işaret hatası)
        const delta = b * b + 4 * c;
        ...
        if (delta >= 0) {
            ...
            if (Number.isInteger(karekoke)) {
                ...
                if (Number.isInteger(p) && Number.isInteger(q) && p !== 0 && q !== 0) {
                    ...
                    // Akıllı Çeldirici 2 (Denge unsuru - p ve q ters işaretlisi)
                    hamYanlislar.push(`(x ${formatla(-p)})(x ${formatla(-q)})`);
                }
            }
        }

        // 3. Üçüncü Grup (c'nin başka çarpanları veya rastgele yakınlar)
        const absC = Math.abs(c);
        for (let i = 2; i < absC; i++) {
            if (absC % i === 0) {
                let r = i;
                let s = absC / i;
                let aday1, aday2;
                if (c > 0) { aday1 = r; aday2 = s; } else { aday1 = r; aday2 = -s; }

                if (Math.abs(aday1) !== Math.abs(m) && Math.abs(aday1) !== Math.abs(n)) {
                    hamYanlislar.push(`(x ${formatla(aday1)})(x ${formatla(aday2)})`);
                    hamYanlislar.push(`(x ${formatla(-aday1)})(x ${formatla(-aday2)})`);
                    break;
                }
            }
        ...
        return {
            metin: `${this.temizlePolinom(soruMetni)} ifadesinin çarpanlarına ayrılmış hali hangisidir?`,
            siklar: this.hazirlaSiklar(hamDogru, hamYanlislar),
...
const Arayuz = {
    ...
    initCanvas: function () {
        ...
        window.addEventListener('resize', () => {
            if (this.canvas && this.canvas.parentElement) {
                // Mevcut çizimi kaybetmemek için bir temp canvas kullanılabilir ama şimdilik basitçe resize
                // Not: Resize canvas içeriğini siler. Basit tutalım.
                ...
                <div class="exam-header">
                    ...
                    <div class="header-center">
                        <span id="exam-speed" class="timer-text">
                            <span class="speed-metric">0 <span class="speed-unit">so/sa</span></span>
                            ...
                            <span class="speed-divider">|</span>
                            <span id="speed-empty" class="speed-metric speed-empty">0 <span class="speed-unit">bo/sa</span></span>
                        </span>
                    </div>
    ...
    sifirlaVeBaslat: function () {
        ...
        if (sEl) {
            ...
            sEl.innerHTML = `<span class="speed-metric">0 <span class="speed-unit">so/sa</span></span>
             <span class="speed-divider">|</span>
             <span id="speed-correct" class="speed-metric speed-correct">0 <span class="speed-unit">do/sa</span></span>
             <span class="speed-divider">|</span>
             <span id="speed-wrong" class="speed-metric speed-wrong">0 <span class="speed-unit">ya/sa</span></span>
        ...
        this.timerInterval = setInterval(() => {
            ...
            if (spEl) spEl.innerHTML = `<span class="speed-metric">${speed} <span class="speed-unit">so/sa</span></span>
            <span class="speed-divider">|</span>
            ...
            <span class="speed-divider">|</span>
            <span class="speed-metric speed-empty">${bosHiz} <span class="speed-unit">bo/sa</span></span>`;
    ...
    stilEkle: function () {
        ...
        style.innerHTML = `
            ...
            .speed-metric {
                font-size: 0.6rem; /* User requested 0.6rem specifically */
                display: flex;
            ...
            .speed-empty { color: #6c757d; } /* Nötr Gri */
