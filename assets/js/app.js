...
window.showPage = (pageIds) => {
    // Hide all
    const pages = ['home-page', 'news-page', 'news-detail', 'games-page', 'lessons-page', 'resources-page', 'columnist-detail', 'quiz-page', 'all-columnists-page', 'contact-page', 'edebiyat-game-page', 'saniye-game-page', 'kuvvet-game-page', 'denklik-game-page', 'story-category-page', 'math-exam-page'];
    pages.forEach(id => {
...
window.showExamPage = (examType) => {
    ...
    // 2. Start Exam Interface (Timer & Reset)
    if (window.Arayuz && window.Arayuz.acilis) {
        window.Arayuz.acilis();
    }

    // 2. Dispatch event (Future proofing, though currently only one exam exists)
